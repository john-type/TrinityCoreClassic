...
def handle_pool_members():
    vm_member_pools = db.vm_world.get_rows_raw("SELECT pool_id, mother_pool, chance, description FROM pool_pool")
...
def _upsert_pool_member(type, spawn_id, pool_spawn_id, chance, desc):
        ...
        if existing == None:
            db.tri_world.execute_raw("INSERT INTO pool_members (type, spawnId, poolSpawnId, chance, description )",
                                 (type, spawn_id, pool_spawn_id, chance, desc,))
        else:
            db.tri_world.execute_raw("UPDATE pool_members SET chance = %s, description = %s WHERE type = %s AND spawnId = %s AND poolSpawnId = %s",
