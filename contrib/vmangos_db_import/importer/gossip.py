...
def _handle_gossip_menu_row(row):
    existing_row = db.tri_world.get_row_raw("SELECT MenuID, TextID FROM gossip_menu WHERE MenuID = %s", (row[0],))
    ...
    if existing_row == None:
        ...
        if (existing_row[0] == row[0] and existing_row[1] == row[1]):
            db.tri_world.execute_raw("UPDATE gossip_menu SET VerifiedBuild = 40618 WHERE MenuID = %s", (row[0],))
