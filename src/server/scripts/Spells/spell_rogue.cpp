        ...
        if (Unit* target = GetHitUnit())
        {
            ...
            for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot)
            {
                ...
                for (uint8 s = 0; s < 3; ++s)
                {
                    ...
                    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(enchant->EffectArg[s], GetCastDifficulty());
                    if (!spellInfo)
                    {
                ...
                return (spellInfo->SpellFamilyFlags[1] & SPELLFAMILYFLAG1_ROGUE_COLDB_SHADOWSTEP ||  // Cold Blood, Shadowstep
                    ...
                    (caster->HasAura(SPELL_ROGUE_GLYPH_OF_PREPARATION) &&
                        (spellInfo->SpellFamilyFlags[1] & SPELLFAMILYFLAG1_ROGUE_DISMANTLE ||            // Dismantle
                            ...
                            (spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG0_ROGUE_BLADE_FLURRY &&          // Blade Flurry
...
class spell_rog_quick_recovery : public AuraScript
{
     ...
     void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (costs.size() > 0)
        {
            int32 amount = CalculatePct(costs[0].Amount, aurEff->GetAmount());
            CastSpellExtraArgs args(aurEff);
...
public:
    ...
    class spell_rog_glyph_of_backstab_triggered_SpellScript : public SpellScript
    {
        ...
        void HandleScript(SpellEffIndex effIndex)
        {
            ...
            {
                RuptureAuraScript* ruptureAuraScript = aurEff->GetBase()->GetScript<RuptureAuraScript>();
...
class spell_rog_tricks_of_the_trade : public SpellScript
{
    ...
    void DoAfterHit()
    {
        if (Aura* aura = GetHitAura())
            if (auto* script = aura->GetScript<spell_rog_tricks_of_the_trade_aura>())
            {
