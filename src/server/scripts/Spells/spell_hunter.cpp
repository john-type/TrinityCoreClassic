    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* unitTarget = GetHitUnit())
        {
            ...
            for (Unit::AuraApplicationMap::const_iterator i = auras.begin(); i != auras.end(); ++i)
            {
                ...
                flag128 familyFlag = aura->GetSpellInfo()->SpellFamilyFlags;
...
class spell_hun_cobra_strikes : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        ...
        SpellInfo const* triggeredSpellInfo = sSpellMgr->AssertSpellInfo(aurEff->GetSpellEffectInfo().TriggerSpell, GetCastDifficulty());
...
class spell_hun_glyph_of_arcane_shot : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        auto costs = procSpell->CalcPowerCost(GetTarget(), procSpell->GetSchoolMask());
        if (costs.size() > 0)
        {
            int32 mana = costs[0].Amount;
...
class spell_hun_feeding_frenzy : public AuraScript
{
    ...
    void PeriodicTick(AuraEffect const* aurEff)
    {
        ...
        if (GetTarget()->GetVictim() && GetTarget()->EnsureVictim()->HasAuraState(AURA_STATE_WOUNDED_35_PERCENT))
...
class spell_hun_lock_and_load : public AuraScript
{
    ...
    {
        if (!(eventInfo.GetTypeMask() & PROC_FLAG_DEAL_HELPFUL_PERIODIC))
            return false;
    ...
    {
        if (!(eventInfo.GetTypeMask() & PROC_FLAG_DEAL_HARMFUL_PERIODIC))
            return false;
...
class spell_hun_masters_call : public SpellScript
{
    ...
    SpellCastResult DoCheckCast()
    {
        ...
        if (pet->GetCharmerGUID() != ObjectGuid::Empty)
            result = SPELL_FAILED_CHARMED;
        else if (pet->HasUnitFlag(UNIT_FLAG_STUNNED))
            result = SPELL_FAILED_STUNNED;
        else if (pet->HasUnitFlag(UNIT_FLAG_FLEEING))
            result = SPELL_FAILED_FLEEING;
        else if (pet->HasUnitFlag(UNIT_FLAG_CONFUSED))
...
class spell_hun_piercing_shots : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        {
            SpellInfo const* piercingShots = sSpellMgr->AssertSpellInfo(SPELL_HUNTER_PIERCING_SHOTS, GetCastDifficulty());
...
class spell_hun_rapid_recuperation_trigger : public AuraScript
{
    ...
    void HandleRapidFireProc(AuraEffect* /*aurEff*/, ProcEventInfo& eventInfo)
    {
    ...
    void HandleRapidKillingProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
...
class spell_hun_tame_beast : public SpellScript
{
    ...
    SpellCastResult CheckCast()
    {
        ...
        if (Creature* target = GetExplTargetUnit()->ToCreature())
        {
            ...
            if (caster->GetCharmedGUID() != ObjectGuid::Empty)
                return SPELL_FAILED_ALREADY_HAVE_CHARM;

            if (target->GetOwnerGUID() != ObjectGuid::Empty)
            {
...
class spell_hun_thrill_of_the_hunt : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        {
            auto costs = spellInfo->CalcPowerCost(caster, spellInfo->GetSchoolMask());
            if (costs.size() > 0)
            {
                amount = CalculatePct(costs[0].Amount, aurEff->GetAmount());
            }
        }
