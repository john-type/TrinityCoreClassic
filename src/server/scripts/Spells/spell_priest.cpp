...
class spell_pri_blessed_recovery : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* triggerInfo = sSpellMgr->AssertSpellInfo(triggerSpell, GetCastDifficulty());
...
class spell_pri_body_and_soul : public AuraScript
{
    ...
    void HandleProcTriggerSpell(AuraEffect* /*aurEff*/, ProcEventInfo& eventInfo)
    {
    ...
    void HandleProcDummy(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        PreventDefaultAction();
...
class spell_pri_improved_spirit_tap : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        {
            if (spellInfo->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_PRIEST_SHADOW_WORD_DEATH_R1, GetCastDifficulty())) ||
                spellInfo->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_PRIEST_MIND_BLAST_R1, GetCastDifficulty())))
                return true;
...
class spell_pri_mind_sear : public SpellScript
{
    ...
    {
        unitList.remove_if(Trinity::ObjectGUIDCheck(GetCaster()->GetChannelObjectGuid(0)));
    }
...
class spell_pri_penance : public SpellScript
{
    ...
    bool Validate(SpellInfo const* spellInfo) override
    {
        SpellInfo const* firstRankSpellInfo = sSpellMgr->GetSpellInfo(SPELL_PRIEST_PENANCE_R1, GetCastDifficulty());
