...
class spell_mage_burnout : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        auto cost = eventInfo.GetDamageInfo()->GetSpellInfo()->CalcPowerCost(GetTarget(), eventInfo.GetDamageInfo()->GetSchoolMask());
        if (cost.size() > 0)
        {
            int32 mana = CalculatePct(cost[0].Amount, aurEff->GetAmount());
...
class spell_mage_imp_blizzard : public AuraScript
{
    ...
    void HandleChill(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        PreventDefaultAction();
...
class spell_mage_empowered_fire : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        ...
        Unit* target = GetTarget();
        CastSpellExtraArgs args(aurEff);
        uint8 percent = sSpellMgr->AssertSpellInfo(SPELL_MAGE_EMPOWERED_FIRE_PROC, GetCastDifficulty())->GetEffect(EFFECT_0).CalcValue();
...
class spell_mage_fingers_of_frost : public AuraScript
{
    ...
    void HandleDummy(AuraEffect* /*aurEff*/, ProcEventInfo& eventInfo)
    {
        PreventDefaultAction();
    ...
    void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (caster->HasAura(SPELL_MAGE_T10_2P_BONUS))
            if (Aura* aura = caster->GetAuraOfRankedSpell(SPELL_MAGE_ARCANE_MISSILES_R1))
                if (spell_mage_arcane_missiles* missiles = aura->GetScript<spell_mage_arcane_missiles>())
