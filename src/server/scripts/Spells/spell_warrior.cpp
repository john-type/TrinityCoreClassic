...
// - Rank 1 - 23881, Rank 2 - 23892, Rank 3 - 23893, Rank 4 - 23894 Bloodthirst
class spell_warr_bloodthirst : public SpellScript
{
    ...
    void HandleDamage(SpellEffIndex /*effIndex*/)
    {
        ...
        uint32 APbonus = caster->GetTotalAttackPowerValue(BASE_ATTACK);
        ...
        // Manually trigger the healing effect
        HandleDummy();
    }
    ...
    void HandleDummy()
    {
        ...
        switch (spellId)
        {
        case SPELL_WARRIOR_BLOODTHIRST_R1: healSpellId = SPELL_WARRIOR_BLOODTHIRST_HEAL_R1; break;
        case SPELL_WARRIOR_BLOODTHIRST_R2: healSpellId = SPELL_WARRIOR_BLOODTHIRST_HEAL_R2; break;
        case SPELL_WARRIOR_BLOODTHIRST_R3: healSpellId = SPELL_WARRIOR_BLOODTHIRST_HEAL_R3; break;
        case SPELL_WARRIOR_BLOODTHIRST_R4: healSpellId = SPELL_WARRIOR_BLOODTHIRST_HEAL_R4; break;
        default: return;
        }

        caster->CastSpell(caster, healSpellId, true);
    }
...
class spell_warr_deep_wounds : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit())
        {
            ...
            SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_WARRIOR_DEEP_WOUNDS_PERIODIC, GetCastDifficulty());
...
class spell_warr_execute : public SpellScript
{
    ...
    void HandleEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit())
        {
            ...
            auto costs = spellInfo->CalcPowerCost(caster, SpellSchoolMask(spellInfo->SchoolMask));
            if (costs.size() > 0) {
                int32 rageUsed = std::min<int32>(300 - costs[0].Amount, caster->GetPower(POWER_RAGE));
...
class spell_warr_extra_proc : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        ...
        if (auraInfo->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_WARRIOR_BLOODSURGE_R1, GetCastDifficulty())))
            target->CastSpell(nullptr, SPELL_WARRIOR_SLAM_GCD_REDUCED, aurEff);
        else if (auraInfo->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_WARRIOR_SUDDEN_DEATH_R1, GetCastDifficulty())))
...
class spell_warr_last_stand : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        Unit* caster = GetCaster();
        CastSpellExtraArgs args(TRIGGERED_FULL_MASK);
        args.AddSpellBP0(caster->CountPctFromMaxHealth(30.f));
...
class spell_warr_rend : public AuraScript
{
    ...
    void CalculateAmount(AuraEffect const* aurEff, int32& amount, bool& canBeRecalculated)
    {
        if (Unit* caster = GetCaster())
        {
            ...
            {
                if (GetUnitOwner()->HasAuraState(AURA_STATE_HEALTHY_75_PERCENT, GetSpellInfo(), caster))
