...
class spell_sha_earthliving_weapon : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (AuraEffect const* aurEff = caster->GetAuraEffectOfRankedSpell(SPELL_SHAMAN_BLESSING_OF_THE_ETERNALS_R1, EFFECT_1, caster->GetGUID()))
            if (eventInfo.GetProcTarget()->HasAuraState(AURA_STATE_WOUNDED_35_PERCENT))
...
class spell_sha_fire_nova : public SpellScript
{
    ...
    bool Validate(SpellInfo const* spellInfo) override
    {
        SpellInfo const* firstRankSpellInfo = sSpellMgr->GetSpellInfo(SPELL_SHAMAN_FIRE_NOVA_R1, DIFFICULTY_NONE);
...
class spell_sha_flametongue_weapon : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        WeaponAttackType attType = Player::GetAttackBySlot(item->GetSlot(), INVTYPE_NON_EQUIP);
...
class spell_sha_frozen_power : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Unit* caster = eventInfo.GetActor();
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_SHAMAN_FREEZE, GetCastDifficulty());
...
class spell_sha_glyph_of_totem_of_wrath : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* totemSpell = sSpellMgr->GetSpellInfo(totem->m_spells[0], GetCastDifficulty());
        if (!totemSpell)
            return;
...
class spell_sha_item_mana_surge : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        auto costs = spellInfo->CalcPowerCost(GetTarget(), eventInfo.GetSchoolMask());
        if (costs.size() > 0)
        {
            CastSpellExtraArgs args(aurEff);
            args.AddSpellBP0(CalculatePct(costs[0].Amount, 35));
...
class spell_sha_item_t10_elemental_2p_bonus : public AuraScript
{
    ...
    void HandleEffectProc(AuraEffect* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        ...
        if (Player* target = GetTarget()->ToPlayer())
            target->GetSpellHistory()->ModifyCooldown(SPELL_SHAMAN_ELEMENTAL_MASTERY, SpellHistory::Duration(-aurEff->GetAmount()), true);
    }
...
class spell_sha_windfury_weapon : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        WeaponAttackType attType = static_cast<WeaponAttackType>(player->GetAttackBySlot(item->GetSlot(), INVTYPE_NON_EQUIP));
        if (attType != BASE_ATTACK && attType != OFF_ATTACK)
            return false;
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_SHAMAN_WINDFURY_WEAPON_R1, GetCastDifficulty());
        while (spellInfo)
        {
        ...
        int32 amount = static_cast<int32>(extraAttackPower / 14.f * player->GetBaseAttackTime(attType) / 1000.f);
