...
class spell_pal_beacon_of_light : public AuraScript
{
    ...
    void PeriodicTick(AuraEffect const* aurEff)
    {
        ...
        CastSpellExtraArgs args(aurEff);
        args.SetOriginalCaster(GetAura()->GetUnitOwner()->GetGUID());
        GetAura()->GetUnitOwner()->CastSpell(GetTarget(), aurEff->GetSpellEffectInfo().TriggerSpell, args);
    }
...
class spell_pal_exorcism_and_holy_wrath_damage : public AuraScript
{
    ...
    void HandleEffectCalcSpellMod(AuraEffect const* aurEff, SpellModifier*& spellMod)
    {
        if (!spellMod)
        {
            ...
            //spellMod->mask[1] = 0x200002; //TODOFROST
        }

        //spellMod->value = aurEff->GetAmount();    //TODOFROST
    }
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (eventInfo.GetHealInfo() && eventInfo.GetHealInfo()->GetSpellInfo())
        {
            ...
            if (eventInfo.GetHealInfo()->GetSpellInfo()->SpellFamilyFlags[1] & 0x00010000)
                originalSpell = sSpellMgr->GetSpellInfo(sSpellMgr->GetSpellWithRank(SPELL_PALADIN_HOLY_SHOCK_R1, eventInfo.GetHealInfo()->GetSpellInfo()->GetRank()), GetCastDifficulty());
            else
            ...
            if (originalSpell && aurEff->GetSpellInfo())
            {
                ...
                auto cost = originalSpell->CalcPowerCost(target, originalSpell->GetSchoolMask());
                if (cost.size() > 0) {
                    uint32 bp = CalculatePct(cost[0].Amount, aurEff->GetSpellInfo()->GetEffect(EFFECT_1).CalcValue());
                    CastSpellExtraArgs args(aurEff);
                    args.AddSpellBP0(bp);
                }
            }
...
class spell_pal_improved_lay_of_hands : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        PreventDefaultAction();
        CastSpellExtraArgs args(aurEff);
        args.SetOriginalCaster(GetTarget()->GetGUID());
        eventInfo.GetActionTarget()->CastSpell(eventInfo.GetActionTarget(), aurEff->GetSpellEffectInfo().TriggerSpell, args);
    }
...
class spell_pal_infusion_of_light : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        //        {
        //            Unit* target = GetTarget();
        //            int32 duration = sSpellMgr->AssertSpellInfo(SPELL_PALADIN_FLASH_OF_LIGHT_PROC, GetCastDifficulty())->GetMaxDuration() / 1000;
...
class spell_pal_judgement_of_wisdom_mana : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_PALADIN_JUDGEMENT_OF_WISDOM_MANA, GetCastDifficulty());
...
class spell_pal_light_s_beacon : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        uint32 healSpellId = procSpell->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_PALADIN_HOLY_LIGHT, GetCastDifficulty())) ? SPELL_PALADIN_BEACON_OF_LIGHT_HEAL_1 : SPELL_PALADIN_BEACON_OF_LIGHT_HEAL_3;
...
class spell_pal_righteous_vengeance : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_PALADIN_RIGHTEOUS_VENGEANCE_DAMAGE, GetCastDifficulty());
...
class spell_pal_seal_of_righteousness : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        float mws = GetTarget()->GetBaseAttackTime(BASE_ATTACK);
        mws /= 1000.0f;
...
public:
    ...
    class spell_pal_seal_of_vengeance_AuraScript : public AuraScript
    {
        ...
        */
        ...
        void HandleApplyDoT(AuraEffect* aurEff, ProcEventInfo& eventInfo)
        {
            PreventDefaultAction();
        ...
        void HandleSeal(AuraEffect* aurEff, ProcEventInfo& eventInfo)
        {
            PreventDefaultAction();
            ...
            if (stacks < maxStacks && !(eventInfo.GetTypeMask() & PROC_FLAG_DEAL_MELEE_ABILITY))
                return;

            SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(DamageSpell, GetCastDifficulty());
...
class spell_pal_sheath_of_light : public AuraScript
{
    ...
    void HandleProc(AuraEffect* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_PALADIN_SHEATH_OF_LIGHT_HEAL, GetCastDifficulty());
        int32 amount = CalculatePct(static_cast<int32>(healInfo->GetEffectiveHeal()), aurEff->GetAmount());
