...
void BattlegroundSA::TeleportToEntrancePosition(Player* player)
{
    if (GetTeamIndexByTeamId(GetPlayerTeam(player->GetGUID())) == Attackers)
    {
        if (!ShipsStarted)
        {
            ...
            if (urand(0, 1))
                player->TeleportTo(607, 2682.936f, -830.368f, 15.0f, 2.895f, TELE_TO_NONE);
            else
                player->TeleportTo(607, 2577.003f, 980.261f, 15.0f, 0.807f, TELE_TO_NONE);
        }
        else
            player->TeleportTo(607, 1600.381f, -106.263f, 8.8745f, 3.78f, TELE_TO_NONE);
    }
    else
        player->TeleportTo(607, 1209.7f, -65.16f, 70.1f, 0.0f, TELE_TO_NONE);
}
